...
int main(int argc, char *argv[]) {
    ...
    solver_ntn = std::make_unique<NTNContactSolverCallback>(*model, "slider_bottom", "base_top", normal_dir, time_step_factor);
    ...
    ///// Set to steady state
    const auto &slider_nodes = mesh->getElementGroup("slider").getNodeGroup().getNodes();
    const auto &base_nodes = mesh->getElementGroup("base").getNodeGroup().getNodes();
    ...
    auto vel_it = make_view(model->getVelocity(), model->getSpatialDimension()).begin();

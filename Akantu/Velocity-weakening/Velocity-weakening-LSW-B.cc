...
int main(int argc, char *argv[]) {
    ...
    if (prank == 0) {
        std::cout << "[DBG] gaps.size = " << contact.getGaps().size() << "\n";
    }
    ...
    // const akantu::Int PRESS_STEPS = 5000;
    ...
    // Goal: close initial gap (~0.5 mm) and add a small overclosure to guarantee contact.
    ...
    for (akantu::Int step_now = 0; step_now < PRESS_STEPS; ++step_now) {
        ...
        if (prank == 0 && step_now % 100 == 0) {
            ...
            akantu::Real gmin = 1e10;
            akantu::Real gmax = -1e10;
            for (auto g : gaps) {
                gmin = std::min(gmin, g);
                gmax = std::max(gmax, g);
            }

            std::cout << "[SIM] Step " << std::setw(8) << step_now << "/" << PRESS_STEPS
                      << " | Gap.size() = " << std::setw(8) << contact.getGaps().size()
                      << " | Max Gap: " << std::setw(8) << std::scientific << std::setprecision(5) << gmax
                      << " | Min Gap: " << std::setw(8) << std::scientific << std::setprecision(5) << gmin
                      << " | Elapsed: " << std::fixed << std::setprecision(1)
                      << std::setw(8) << elapsed.count() << " s"
                      << " | Progress: " << std::setw(6)
                      << std::fixed << std::setprecision(1) << percent_press << " %"
    ...
    for (akantu::Int step_now = 0; step_now < SHEAR_STEPS; ++step_now) {
        ...
        if (prank == 0 && step_now % 100 == 0) {
            std::cout << "[SIM] Step " << std::setw(8) << step_now << "/" << SHEAR_STEPS
                      ...
                      << std::setw(8) << elapsed.count() << " s"
                      << " | Progress: " << std::setw(6)
                      << std::fixed << std::setprecision(1) << percent_shear << " %"
                      << " | ETA: " << std::fixed << std::setprecision(1)
                      << std::setw(8) << std::max(0.0, remaining) << " s\n";
        }
    }

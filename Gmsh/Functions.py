...
def create_block_B(origin, dimensions, mesh_size, block_name, tag_prefix=1):
    x, y, z = origin
    dx, dy, dz = dimensions
...
def create_block(origin, dimensions, mesh_size, block_name, tag_prefix=1):
    ...
    for dim_s, tag_s in faces:
        curves = gmsh.model.getBoundary([(dim_s, tag_s)], oriented=False)
        for dim_c, tag_c in curves:
            if dim_c != 1:
                continue
            L = gmsh.model.occ.getMass(dim_c, tag_c)
            n = max(2, int(round(L / mesh_size)) + 1)
            gmsh.model.mesh.setTransfiniteCurve(tag_c, n)

        gmsh.model.mesh.setTransfiniteSurface(tag_s)
        gmsh.model.mesh.setRecombine(dim_s, tag_s)
    ...
    for dim, tag in faces:
        com = gmsh.model.occ.getCenterOfMass(dim, tag)
        if abs(com[0] - x) < tolerance:
            name = f"{block_name}-back";   tag_val = tag_prefix * 10 + 4
        elif abs(com[0] - (x + dx)) < tolerance:
            name = f"{block_name}-front";  tag_val = tag_prefix * 10 + 5
        elif abs(com[1] - y) < tolerance:
            name = f"{block_name}-right";  tag_val = tag_prefix * 10 + 6
        elif abs(com[1] - (y + dy)) < tolerance:
            name = f"{block_name}-left";   tag_val = tag_prefix * 10 + 7
        elif abs(com[2] - z) < tolerance:
            name = f"{block_name}-bottom"; tag_val = tag_prefix * 10 + 2
        elif abs(com[2] - (z + dz)) < tolerance:
            name = f"{block_name}-top";    tag_val = tag_prefix * 10 + 3
        else:
            continue
